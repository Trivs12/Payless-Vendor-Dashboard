...
const findTopSKU = (skuData) => {
  ...
  for (const [sku, data] of Object.entries(skuData)) {
    const growth = ((data.current - data.previous) / Math.abs(data.previous || 1)) * 100;
    if (growth > maxGrowth) {
      maxGrowth = growth;
      topSku = { sku, label: data.label, growth, current: data.current, previous: data.previous };
    }
  }
  return topSku;
};
...
export const generateYoYInsights = (yoyData) => {
  ...
  if (peakMonth) {
    const growth = formatPct(peakMonth.current.totalSales, peakMonth.previous.totalSales);
    const monthName = peakMonth.month;
    sentences.push(
      `${monthName} demonstrated the strongest performance with ${growth} revenue growth, reaching ${formatCurrency(peakMonth.current.totalSales)}.`
    );
  }
...
export const generateExecutiveSummary = (yoyData, campaignData) => {
  ...
  const totalNewCustomers = campaignData.totalNewCustomers || 0;
  ...
  const returnCustGrowth = campaignData.prevReturningCustomers
    ? formatPct(campaignData.totalReturningCustomers, campaignData.prevReturningCustomers)
    : 'N/A';
