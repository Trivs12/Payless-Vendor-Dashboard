...
/**
 * Extract YYYY-MM-DD from a date string (handles various formats)
 */
function extractDate(dateStr) {
  ...
  const match2 = dateStr.match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/);
  if (match2) {
    return `${match2[3]}-${String(match2[1]).padStart(2, '0')}-${String(match2[2]).padStart(2, '0')}`;
  }
  return null;
}
...
export function parseProductCSV(csvText, skuMap = {}) {
  ...
  // Second pass: aggregate data by month and by day
  result.data.forEach((row) => {
    const month = extractMonth(row['Day']);
    const day = extractDate(row['Day']);
    if (!month) return;

    monthsSet.add(month);
    const sku = row['Product variant SKU'];

    // Aggregate daily totals (across all SKUs per day)
    if (day) {
      if (!dailyTotals[day]) {
        dailyTotals[day] = {
          totalSales: 0,
      ...
      dailyTotals[day].totalSales += parseNumeric(row['Total sales']);
      dailyTotals[day].netItems += parseNumeric(row['Net items sold']);
      dailyTotals[day].newCustomers += parseNumeric(row['New customers']);
      dailyTotals[day].returningCustomers += parseNumeric(row['Returning customers']);
      dailyTotals[day].prevTotalSales += parseNumeric(row['Total sales (previous_year)']);
      dailyTotals[day].prevNetItems += parseNumeric(row['Net items sold (previous_year)']);
      dailyTotals[day].prevNewCustomers += parseNumeric(row['New customers (previous_year)']);
      dailyTotals[day].prevReturningCustomers += parseNumeric(row['Returning customers (previous_year)']);
    }

    if (!sku || !sku.trim()) return;

    const productTitle = (row['Product title at time of sale'] || row['Product title'] || '').trim();
    const variantTitle = (row['Product variant title'] || row['Variant title'] || '').trim();
    ...
    // Build a display label from product title + variant title
    let skuLabel = skuMap[sku] || sku;
    if (productTitle) {
...
/**
 * Parse Customer CSV data with per-customer-ID deduplication
 * Each row is one customer per day with binary 0/1 flags for new/returning.
 * Returning customers are deduplicated per month (counted once per unique Customer ID).
 * New customers are also deduplicated per month for safety.
 * Expected columns: "Day", "Customer ID", "New customers", "Returning customers",
 *   "Day (previous_year)", "New customers (previous_year)", "Returning customers (previous_year)"
 */
export function parseCustomerCSV(csvText) {
  ...
  if (result.errors.length > 0) {
    console.warn('Customer CSV parsing errors:', result.errors);
  }

  // Track unique customer IDs per month per type
  // Current year: keyed by current month
  // Previous year: keyed by current month (so it aligns with the same report month)
  const currentNew = {};     // month -> Set of customer IDs
  const currentReturning = {};
  const prevNew = {};
  const prevReturning = {};
  ...
  result.data.forEach((row) => {
    ...
    const month = extractMonth(day);
    if (!month || !customerId) return;

    // Initialize sets for this month
    if (!currentNew[month]) {
      currentNew[month] = new Set();
      currentReturning[month] = new Set();
      prevNew[month] = new Set();
      prevReturning[month] = new Set();
    }

    // Current year
    if (parseNumeric(row['New customers']) >= 1) {
      currentNew[month].add(customerId);
    }
    if (parseNumeric(row['Returning customers']) >= 1) {
      currentReturning[month].add(customerId);
    }

    // Previous year (use the same current month key so data aligns)
    if (parseNumeric(row['New customers (previous_year)']) >= 1) {
      prevNew[month].add(customerId);
    }
    // Handle both with and without trailing space in column name
    const prevRetVal = row['Returning customers (previous_year)'] || row['Returning customers (previous_year) '];
    if (parseNumeric(prevRetVal) >= 1) {
      prevReturning[month].add(customerId);
    }
  });
  ...
  const allMonths = new Set([
    ...Object.keys(currentNew),
    ...Object.keys(prevNew),
  ]);

  allMonths.forEach((month) => {
    monthlyData[month] = {
      newCustomers: currentNew[month]?.size || 0,
      returningCustomers: currentReturning[month]?.size || 0,
      prevNewCustomers: prevNew[month]?.size || 0,
      prevReturningCustomers: prevReturning[month]?.size || 0,
    };
  });

  // Compute period-level unique returning customers (deduplicated across all months)
  const allReturning = new Set();
  const allPrevReturning = new Set();
  Object.values(currentReturning).forEach((s) => s.forEach((id) => allReturning.add(id)));
  Object.values(prevReturning).forEach((s) => s.forEach((id) => allPrevReturning.add(id)));

  return {
    monthly: monthlyData,
    periodTotals: {
      uniqueReturningCustomers: allReturning.size,
      prevUniqueReturningCustomers: allPrevReturning.size,
    },
  };
}
...
export function calculateYoY(monthlyTotals, monthlySkuData, months) {
  ...
  months.forEach((month) => {
    ...
    Object.entries(monthlySkuData[month] || {}).forEach(([sku, skuMetrics]) => {
      skuData[sku] = {
        label: skuMetrics.skuLabel || sku,
        current: skuMetrics.totalSales || 0,
        previous: skuMetrics.prevTotalSales || 0,
        change: (skuMetrics.totalSales || 0) - (skuMetrics.prevTotalSales || 0),
        currentItems: skuMetrics.netItems || 0,
        previousItems: skuMetrics.prevNetItems || 0,
        itemsChange: (skuMetrics.netItems || 0) - (skuMetrics.prevNetItems || 0),
      };
    });
    ...
    comparisons.push({
      ...
      current: {
        ...
        netItems: data.netItems || 0,
        newCustomers: data.newCustomers || 0,
        returningCustomers: data.returningCustomers || 0,
      },
      previous: {
        totalSales: data.prevTotalSales || 0,
        netItems: data.prevNetItems || 0,
        newCustomers: data.prevNewCustomers || 0,
        returningCustomers: data.prevReturningCustomers || 0,
      },
      change,
      skuData,
    });
