...
/**
 * Parse Customer CSV data (deduplicated new/returning customers by day)
 * Aggregates daily counts into monthly totals with current and previous year
 * Expected columns: "New or returning customer", "Day", "Customers", "Day (previous_year)", "Customers (previous_year)"
 */
export function parseCustomerCSV(csvText) {
  ...
  result.data.forEach((row) => {
    const type = (row['New or returning customer'] || '').trim();
    const day = row['Day'];
    const customers = parseNumeric(row['Customers']);
    const prevCustomers = parseNumeric(row['Customers (previous_year)']);
    ...
    if (type.toLowerCase() === 'new') {
      monthlyData[month].newCustomers += customers;
      monthlyData[month].prevNewCustomers += prevCustomers;
    } else if (type.toLowerCase() === 'returning') {
      monthlyData[month].returningCustomers += customers;
      monthlyData[month].prevReturningCustomers += prevCustomers;
    }
  });
