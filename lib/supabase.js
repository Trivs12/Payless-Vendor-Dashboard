...
// SKU title mapping
export async function getSkuTitleMap(vendorId) {
  if (!supabase) return [];
  const { data, error } = await supabase
    .from('sku_title_map')
    .select('*')
...
export async function saveSkuTitleMap(vendorId, mappings) {
  if (!supabase) throw new Error('Database not configured');
  // Upsert all mappings
  ...
  if (rows.length > 0) {
    const { error } = await supabase
      .from('sku_title_map')
      .upsert(rows, { onConflict: 'vendor_id,sku' });
    if (error) throw error;
  }
}

export async function deleteSkuTitleMapping(vendorId, sku) {
  if (!supabase) throw new Error('Database not configured');
  const { error } = await supabase
    .from('sku_title_map')
    .delete()
