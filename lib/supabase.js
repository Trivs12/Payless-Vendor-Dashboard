...
// ==================== Customer Data (deduplicated, separate from product data) ====================

export async function saveCustomerData(reportId, customerData) {
  if (!supabase) throw new Error('Database not configured');
  // Delete existing customer data for this report
  await supabase.from('monthly_customer_data').delete().eq('report_id', reportId);
  const rows = Object.entries(customerData).map(([month, data]) => ({
    report_id: reportId,
    month,
    new_customers: data.newCustomers || 0,
    returning_customers: data.returningCustomers || 0,
    prev_new_customers: data.prevNewCustomers || 0,
    prev_returning_customers: data.prevReturningCustomers || 0,
    returning_customer_ids: data.returningCustomerIds || [],
    prev_returning_customer_ids: data.prevReturningCustomerIds || [],
  }));
  if (rows.length > 0) {
    const { error } = await supabase.from('monthly_customer_data').insert(rows);
    if (error) throw error;
  }
  return rows.length;
}

export async function getCustomerData(reportId) {
  if (!supabase) return [];
  const { data, error } = await supabase
